...
do
        ...
        do
                ...
                then
                        echo "The impala query submitted by user $user is running for $(expr $difference / 60 ):$(expr $difference % 60)). Please check it." | mail -s "Warning: $user Impala query running for more than $(expr $THRESHOLD_TIME / 60 )  mins" -r $SUPPORT_EMAIL "$SUPPORT_EMAIL"
                fi
        done
done
